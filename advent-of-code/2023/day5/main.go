...
func part2(lines []string) {
	...
	for charIndex < len(seeds[1]) {
		if string(seeds[1][charIndex]) != " " {
			...
			seed2, lastIndex2 := getFullNumber(seeds[1], charIndex)
			charIndex = lastIndex2
			for i := seed; i <= seed+seed2-1; i++ {
				trackList = append(trackList, i)
			}
			minimum := getMinLocation(lines, trackList)
			trackList = []int{}
	...
	minLoc := 9223372036854775807
	for _, val := range minList {
		if val < minLoc {
			minLoc = val
		}
	}
...
func getMinLocation(lines []string, trackList []int) int {
	mapNum := 1
	for _, line := range lines[2:] {
		if line == "" {
			for trackedIndex, tracked := range trackList {
